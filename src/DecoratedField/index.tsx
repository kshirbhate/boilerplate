...
export const FieldDecorations = props => {
  ...
  const value = props.tstype === 'slider' && props.range ? (Array.isArray(props.input.value) ? props.input.value : [0]) : props.input.value;
  return (
    <FormGroup fieldClassName={props.fieldClassName}>
      <FormField htmlFor={id} {...props} {...props.input} type={props.tstype} format={props.dateFormat} value={value} error={touched ? error : null} />
    </FormGroup>
  );
};
